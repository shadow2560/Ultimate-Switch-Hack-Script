...
mkdir templogs
IF NOT EXIST "tools\sd_switch\salty-nx\profiles\*.*" mkdir "tools\sd_switch\salty-nx\profiles"
...
copy nul "tools\sd_switch\salty-nx\profiles\%new_profile_name%.ini" >nul
...
IF %errorlevel% EQU 0 (
	...
	:removing_profile
	del /q "tools\sd_switch\salty-nx\profiles\%profile_selected%" >nul
...
IF NOT EXIST "tools\sd_switch\salty-nx\profiles\*.ini" exit /b 404
set /a temp_count=1
