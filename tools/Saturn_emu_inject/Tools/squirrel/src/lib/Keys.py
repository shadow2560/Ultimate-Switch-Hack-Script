...
for i in range(1,len(sys.argv), 1):
	...
	if currArg == '-k' or currArg == '--keyset':
		raw_keys_file = Path(sys.argv[i+1])
		break

if sq_settings.key_system =="production":
	raw_keys_file2 = Path('keys.txt')
	raw_keys_file3 = Path('ztools\\keys.txt')
	raw_keys_file4 = Path('ztools/keys.txt')
else:
	raw_keys_file2 = Path('dev_keys.txt')
	raw_keys_file3 = Path('ztools\\dev_keys.txt')
	raw_keys_file4 = Path('ztools/keys.txt')	
