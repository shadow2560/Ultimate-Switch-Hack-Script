...
void die ( char * error , int quiet)
{
    ...
    if (quiet == 0)
    {    
        printf ( "\nPress any keys to close the program" );
        getch();
    }
	exit(dw);
...
void write_sect ( HANDLE hDevice, DWORD Sector, DWORD BytesPerSector, void *Data, DWORD NumSects , int quiet)
{
	DWORD dwWritten;
    BOOL ret;
    ...
    if ( !ret )
        die ( "Failed to write" , quiet);
}

void zero_sectors ( HANDLE hDevice, DWORD Sector, DWORD BytesPerSect, DWORD NumSects, DISK_GEOMETRY* pdgDrive  , int quiet)
{
    BYTE *pZeroSect;
    DWORD BurstSize;
    DWORD WriteSize;
...
int format_volume ( char vol, format_params* params , int quiet)
{
    ...
    if (quiet == 0)
    {
        ...
        if ( toupper(c) != 'Y' )
        {
            printf( "Formating has been canceled.\n" );
			if (quiet == 0)
            {    
                printf ( "\nPress any keys to close the program" );
                getch();
    ...
    if ( !bRet )
        die( "Failed to lock device" , quiet);
    ...
    if ( !bRet )
        die( "Failed to get device geometry" , quiet);
	...
	if ( !bRet )
    {
		...
		if (!bRet)
			die( "Failed to get partition info (both regular and _ex)" , quiet);
    ...
    if ( !pFAT32BootSect || !pFAT32FsInfo || !pFirstSectOfFat )
        die ( "Failed to allocate memory" , quiet);
	...
	*/
    ...
    zero_sectors( hDevice, 0, BytesPerSect, SystemAreaSize, &dgDrive, quiet);
    ...
    for ( i=0; i<2; i++ )
        ...
        write_sect ( hDevice, SectorStart, BytesPerSect, pFAT32BootSect, 1 , quiet);
        write_sect ( hDevice, SectorStart+1, BytesPerSect, pFAT32FsInfo, 1 , quiet);
        }
    ...
    for ( i=0; i<NumFATs; i++ )
        ...
        write_sect ( hDevice, SectorStart, BytesPerSect, pFirstSectOfFat, 1 , quiet);
        }
	...
	if ( !bGPTMode )
		...
		if ( !bRet )
			...
			if ( piDrive.HiddenSectors  )
				die( "Failed to set partition info" , quiet);
			}    
		}
...
void usage( int quiet)
{
        printf ( "Usage Fat32Format X:\n\n" );
        printf ( "Erase all data on disk X:, format it for FAT32\n" );
        printf( "Fat32Format X:  - Format volume X with default params\n\n" );
        printf( "It is possible to ignore the confirmation\n" );
        printf( "Fat32Format -q X:  - Format volume X without confirmation\n\n" );
        ...
        printf( "You can conbine the two params '-q' and '-c', only the volume letter should be passed for the  last param\n" );
        printf( "Fat32Format -q -c64 X:  - use 64 sectors per cluster and format without confirmation\n" );
        printf( "Fat32Format -c32 -q X:  - use 32 sectors per cluster and format without confirmation\n\n" );
        printf ( "Version 1.08, see http://www.ridgecrop.demon.co.uk/fat32format.htm \n" );
        printf ( "This software is covered by the GPL \n" );
        printf ( "Use with care - Ridgecrop are not liable for data lost using this tool \n" );
        if (quiet == 0)
        {    
            printf ( "Press any keys to close the program" );
            getch();
        }
		exit(1);
}
