...
:display_title
title Homebrews profiles management %this_script_version% - Shadow256 Ultimate Switch Hack Script %ushs_version%
goto:eof

:main_action_choice
echo Homebrews profiles management
echo.
...
echo 0: List homebrews on a profile?
...
echo No homebrews in this profile.
goto:eof

:intro_homebrews_one_page
echo Select a homebrew to add or remove it in the profile "%temp_profile:~0,-4%"
goto:eof

:intro_homebrews_multi_page
echo Select a homebrew to add or remove it in the profile "%temp_profile:~0,-4%", page %selected_page%/%page_number%
goto:eof

:add_remove_homebrews_info
echo The homebrews prefixed by an "*" are the homebrews present in the profile.
goto:eof
...
echo All other choices: Stop the homebrews list modification of the profile.
echo.
set /p homebrew_choice=Choose a homebrew to add/remove it or select an other page: 
goto:eof
...
echo Error, no homebrews in the "tools\sd_switch\mixed\modular" folder of the script, the process couldn't continue.
goto:eof
