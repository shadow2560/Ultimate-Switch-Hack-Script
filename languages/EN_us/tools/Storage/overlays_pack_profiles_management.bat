...
:display_title
title overlays profiles management %this_script_version% - Shadow256 Ultimate Switch Hack Script %ushs_version%
goto:eof

:main_action_choice
echo overlays profiles management
echo.
...
echo 0: List overlays on a profile?
...
echo No overlays in this profile.
goto:eof

:intro_overlays_one_page
echo Select a overlay to add or remove it in the profile "%temp_profile:~0,-4%"
goto:eof

:intro_overlays_multi_page
echo Select a overlay to add or remove it in the profile "%temp_profile:~0,-4%", page %selected_page%/%page_number%
goto:eof

:add_remove_overlays_info
echo The overlays prefixed by an "*" are the overlays present in the profile.
goto:eof
...
echo All other choices: Stop the overlays list modification of the profile.
echo.
set /p overlay_choice=Choose a overlay to add/remove it or select an other page: 
goto:eof
