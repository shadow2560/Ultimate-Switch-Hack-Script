...
:display_title
title Modules profiles management %this_script_version% - Shadow256 Ultimate Switch Hack Script %ushs_version%
goto:eof

:main_action_choice
echo Modules profiles management
echo.
...
echo 0: List modules on a profile?
...
echo No modules in this profile.
goto:eof

:intro_modules_one_page
echo Select a module to add or remove it in the profile "%temp_profile:~0,-4%"
goto:eof

:intro_modules_multi_page
echo Select a module to add or remove it in the profile "%temp_profile:~0,-4%", page %selected_page%/%page_number%
goto:eof

:add_remove_modules_info
echo The modules prefixed by an "*" are the modules present in the profile.
goto:eof
...
echo All other choices: Stop the modules list modification of the profile.
echo.
set /p module_choice=Choose a module to add/remove it or select an other page: 
goto:eof
...
echo Error, no emulators in the "tools\sd_switch\modules\pack" folder of the script, the process couldn't continue.
goto:eof
