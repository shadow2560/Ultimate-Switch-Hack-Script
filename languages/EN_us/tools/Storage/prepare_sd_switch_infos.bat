...
IF /i "%copy_atmosphere_pack%"=="o" (
	...
	IF /i NOT "%atmosphere_manual_config%"=="o" (
		...
		IF /i "%atmo_default_auto_upload_global_setting%"=="o" (
			echo Upload infos of global settings to Nintendo servers enabled.
		) else (
			echo Upload infos of global settings to Nintendo servers disabled.
		)
...
IF /i "%sphaira_replace_hbmenu%"=="o" (
	echo Sphaira homebrew will replace the Homebrew Menu.
) else (
	echo Sphaira homebrew will not replace the Homebrew Menu.
)
echo.
IF /i "%copy_emu%"=="o" (
	echo Emulators:
